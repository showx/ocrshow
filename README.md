<p align="center">
  <img src="docs/logo.svg" width="88" height="88" alt="ocrshow" />
</p>

<h1 align="center">ocrshow</h1>

<p align="center">
  <strong>把表格截图识别成结构化记录</strong><br>
  <span>PaddleOCR 读表 · 插件化版式拆字段 · 可选 Qwen3-VL 对照原图纠错</span>
</p>
...
<p align="center">
  ...
  <img alt="PaddleOCR" src="https://img.shields.io/badge/PaddleOCR-PP--StructureV3-2932E1" />
  <img alt="Qwen3-VL" src="https://img.shields.io/badge/Qwen3--VL-optional-1a1a2e" />
</p>

<p align="center">
  <a href="#快速开始">快速开始</a> ·
  <a href="#web-工作台">Web</a> ·
  <a href="#命令行">CLI</a> ·
  <a href="#版式插件">版式插件</a> ·
  <a href="#开源边界">开源边界</a>
</p>
...
<p align="center">
  <img src="docs/screenshot-home.svg" alt="上传截图、选择版式、查看任务队列" width="920" />
</p>

清单、后台、报表截图丢进去，得到 JSON / CSV / Excel。开源核心是 **PaddleOCR PP-StructureV3** 做表格 OCR，再按已安装的版式模块拆字段；准确率不够时可以再开 **Qwen3-VL**，对照原图纠错。
...
| | |
|---|---|
| **拖一张图就能跑** | Web 上传排队，或 CLI 扫目录。GPU 上一次只跑一个任务，避免抢卡。 |
| **自动认版式** | 按表头匹配已安装模块；没有命中就走通用表格，把识别框导出成文本行。 |
| **插件化，而不是写死** | 开源仓库只带骨架。内部解析是 drop-in 模块，不必改 `pipeline.py`。 |
| **公私分离** | 截图、密钥、识别结果、内部版式都留在本地，公开仓库只提交通用代码。 |

<p align="center">
  <img src="docs/architecture.svg" alt="截图 → PP-StructureV3 → 版式模块 → JSON / CSV / Excel" width="920" />
</p>

## 界面

上传后进入任务页。识别完成会列出结构化表格，可导出 JSON；内部模块装上后，列名会跟着版式走。

<p align="center">
  <img src="docs/screenshot-result.svg" alt="识别结果表与原图缩略图" width="920" />
</p>
...
浏览器  →  Vue（:5173）  →  Go API（:8080）  →  SQLite / 上传目录
                              ...
                              pipeline.py
                                    ↓
              PaddleOCR PP-StructureV3     可选 Qwen3-VL（Ollama）
                                    ↓
                    sheets（generic + 可选 private）
                                    ↓
                         JSON / CSV / Excel
```
...
把待识别截图放到 `samples/`。命令行不传路径时默认扫这个目录。
...
一键脚本会用 uv 准备 Python 3.11 虚拟环境、PaddleOCR、Go 模块和前端 npm。Web 还需要 Go 1.21+、Node.js 18+：Windows 缺省时尝试 winget；Linux 缺省时下载到 `~/.local/go` 和 `~/.local/node`（无需 root）。Linux 上 PaddleOCR 还依赖 `libGL` 等系统库，有免密 sudo 时会尝试 `apt-get install`。
...
开源仓库只带自动识别骨架和 `generic`。内部解析放到 `sheets/private/`（已被 gitignore），拷进来即可拼接：

```
sheets/
  generic.py / generic.toml     # 开源兜底
  private/                      # 不入库
    my_sheet.py                 # register(MySheet())
    my_sheet.toml               # 与 py 的 id 一致，Web 才会出现按钮
```

写法、接口和最小例子见 [`sheets/README.md`](sheets/README.md)。
...
| README、`docs/` 示意、依赖清单 | `data/`、`output/`、模型、xlsx/csv 结果 |

Web 上传的文件写在 `data/uploads/`，同样不会入库。`samples/` 目录会进 git，**里面的图片不会**。

## 致谢

- [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR) PP-StructureV3
- [Qwen](https://github.com/QwenLM/Qwen3-VL) / [Ollama](https://ollama.com/)
- Vue 3、Go、SQLite
